<template>
  <div class="filmslist">
    <ul>
      ...
      <router-link
        :to="{name:'filmlink',params:{id:subject.id}}"
      ...
      >
        ...
        <p>{{subject.original_title}}</p>
        <p>
          <span v-for="(genre,index) in subject.genres" :key="index">{{genre}}</span>
        </p>
      </router-link>
    </ul>
  </div>
...
export default {
  ...
  async created() {
    let {
      data: { subjects }
    } = await this.$axios.get("/api/v2/movie/top250?count=20&&start=0");
